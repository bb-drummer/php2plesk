...
##
install_basics () {
	logMessage ">>> begin installing basic tools and/or libraries needed for build process...";
	if [ "$INST_VERBOSE" == "1" ]; then 
		echo "";
	    echo "================================================================================================";
	    echo ">>> begin installing basic tools and/or libraries needed for build process...";
	    echo ""; 
	fi
	...
	if [ "$INST_VERBOSE" == "1" ]; then
		...
		if dpkg --list | grep "libxml2-dev" >/dev/null; then echo "'libxml2-dev' already installed";
		else apt-get install -y libxml2-dev; fi
		
		if dpkg --list | grep "libcurl4-openssl-dev" >/dev/null; then echo "'libcurl4-openssl-dev' already installed";
		else apt-get install -y libcurl4-openssl-dev; fi
		if dpkg --list | grep "pkg-config" >/dev/null; then echo "'pkg-config' already installed";
		else apt-get install -y pkg-config; fi
		
		if dpkg --list | grep "libbz2-dev" >/dev/null; then echo "'libbz2-dev' already installed";
		else apt-get install -y libbz2-dev; fi
	
		if dpkg --list | grep "libjpeg62" >/dev/null; then echo "'libjpeg62' already installed";
		else apt-get install -y libjpeg62; fi
		if dpkg --list | grep "libjpeg62-dev" >/dev/null; then echo "'libjpeg62-dev' already installed";
		else apt-get install -y libjpeg62-dev; fi
	
		if dpkg --list | grep "libpng12-0" >/dev/null; then echo "'libpng12-0' already installed";
		else apt-get install -y libpng12-0; fi
		if dpkg --list | grep "libpng12-dev" >/dev/null; then echo "'libpng12-dev' already installed";
		else apt-get install -y libpng12-dev; fi
	
		if dpkg --list | grep "libfreetype6" >/dev/null; then echo "'libfreetype6' already installed";
		else apt-get install -y libfreetype6; fi
		if dpkg --list | grep "libfreetype6-dev" >/dev/null; then echo "'libfreetype6-dev' already installed";
		else apt-get install -y libfreetype6-dev; fi
	
		if dpkg --list | grep "libgmp3-dev" >/dev/null; then echo "'libgmp3-dev' already installed";
		else apt-get install -y libgmp3-dev; fi
	
		if dpkg --list | grep "libc-client2007e" >/dev/null; then echo "'libc-client2007e' already installed";
		else apt-get install -y libc-client2007e; fi
		if dpkg --list | grep "libc-client2007e-dev" >/dev/null; then echo "'libc-client2007e-dev' already installed";
		else apt-get install -y libc-client2007e-dev; fi
	
		if dpkg --list | grep "libicu-dev" >/dev/null; then echo "'libicu-dev' already installed";
		else apt-get install -y libicu-dev; fi
	
		if dpkg --list | grep "libsasl2-2" >/dev/null; then echo "'libsasl2-2' already installed";
		else apt-get install -y libsasl2-2; fi
		if dpkg --list | grep "libsasl2-dev" >/dev/null; then echo "'libsasl2-dev' already installed";
		else apt-get install -y libsasl2-dev; fi
		if dpkg --list | grep "libsasl2-modules" >/dev/null; then echo "'libsasl2-modules' already installed";
		else apt-get install -y libsasl2-modules; fi
	
		if dpkg --list | grep "libmcrypt4" >/dev/null; then echo "'libmcrypt4' already installed";
		else apt-get install -y libmcrypt4; fi
		if dpkg --list | grep "libmcrypt-dev" >/dev/null; then echo "'libmcrypt-dev' already installed";
		else apt-get install -y libmcrypt-dev; fi
	
		if dpkg --list | grep "unixodbc-dev" >/dev/null; then echo "'unixodbc-dev' already installed";
		else apt-get install -y unixodbc-dev; fi
	
		if dpkg --list | grep "libmysqlclient15-dev" >/dev/null; then echo "'libmysqlclient15-dev' already installed";
		else apt-get install -y libmysqlclient15-dev; fi
	
		if dpkg --list | grep "libpq-dev" >/dev/null; then echo "'libpq-dev' already installed";
		else apt-get install -y libpq-dev; fi
	
		if dpkg --list | grep "libpspell-dev" >/dev/null; then echo "'libpspell-dev' already installed";
		else apt-get install -y libpspell-dev; fi
	
		if dpkg --list | grep "libsnmp-dev" >/dev/null; then echo "'libsnmp-dev' already installed";
		else apt-get install -y libsnmp-dev; fi
	
		if dpkg --list | grep "libxslt1-dev" >/dev/null; then echo "'libxslt1-dev' already installed";
		else apt-get install -y libxslt1-dev; fi
		if dpkg --list | grep "libxslt1.1" >/dev/null; then echo "'libxslt1.1' already installed";
		else apt-get install -y libxslt1.1; fi
	
		if [ $INST_MEMCACHED == 1 ] 
			then
				if dpkg --list | grep "libevent" >/dev/null; then echo "'libevent' already installed";
				else apt-get install -y libevent; fi
				if dpkg --list | grep "libevent-dev" >/dev/null; then echo "'libevent-dev' already installed";
				else apt-get install -y libevent-dev; fi
				if dpkg --list | grep "memcached" >/dev/null; then echo "'memcached' already installed";
				else apt-get install -y memcached; fi
				if dpkg --list | grep "libmemcached" >/dev/null; then echo "'libmemcached' already installed";
				else apt-get install -y libmemcached; fi
				if dpkg --list | grep "libmemcached-devel" >/dev/null; then echo "'libmemcached-devel' already installed";
				else apt-get install -y libmemcached-devel; fi
		fi
		...
		if dpkg --list | grep "make" >/dev/null; then echo "'make' already installed" >/dev/null;
		else apt-get install -y make >/dev/null; fi
	
		if dpkg --list | grep "autoconf" >/dev/null; then echo "'autoconf' already installed" >/dev/null;
		else apt-get install -y autoconf >/dev/null; fi

		if dpkg --list | grep "libxml2-dev" >/dev/null; then echo "'libxml2-dev' already installed" >/dev/null;
		else apt-get install -y libxml2-dev >/dev/null; fi
		
		if dpkg --list | grep "libcurl4-openssl-dev" >/dev/null; then echo "'libcurl4-openssl-dev' already installed" >/dev/null;
		else apt-get install -y libcurl4-openssl-dev >/dev/null; fi
		if dpkg --list | grep "pkg-config" >/dev/null; then echo "'pkg-config' already installed" >/dev/null;
		else apt-get install -y pkg-config >/dev/null; fi
		
		if dpkg --list | grep "libbz2-dev" >/dev/null; then echo "'libbz2-dev' already installed" >/dev/null;
		else apt-get install -y libbz2-dev >/dev/null; fi
	
		if dpkg --list | grep "libjpeg62" >/dev/null; then echo "'libjpeg62' already installed" >/dev/null;
		else apt-get install -y libjpeg62 >/dev/null; fi
		if dpkg --list | grep "libjpeg62-dev" >/dev/null; then echo "'libjpeg62-dev' already installed" >/dev/null;
		else apt-get install -y libjpeg62-dev >/dev/null; fi
	
		if dpkg --list | grep "libpng12-0" >/dev/null; then echo "'libpng12-0' already installed" >/dev/null;
		else apt-get install -y libpng12-0 >/dev/null; fi
		if dpkg --list | grep "libpng12-dev" >/dev/null; then echo "'libpng12-dev' already installed" >/dev/null;
		else apt-get install -y libpng12-dev >/dev/null; fi
	
		if dpkg --list | grep "libfreetype6" >/dev/null; then echo "'libfreetype6' already installed" >/dev/null;
		else apt-get install -y libfreetype6 >/dev/null; fi
		if dpkg --list | grep "libfreetype6-dev" >/dev/null; then echo "'libfreetype6-dev' already installed" >/dev/null;
		else apt-get install -y libfreetype6-dev >/dev/null; fi
	
		if dpkg --list | grep "libgmp3-dev" >/dev/null; then echo "'libgmp3-dev' already installed" >/dev/null;
		else apt-get install -y libgmp3-dev >/dev/null; fi
	
		if dpkg --list | grep "libc-client2007e" >/dev/null; then echo "'libc-client2007e' already installed" >/dev/null;
		else apt-get install -y libc-client2007e >/dev/null; fi
		if dpkg --list | grep "libc-client2007e-dev" >/dev/null; then echo "'libc-client2007e-dev' already installed" >/dev/null;
		else apt-get install -y libc-client2007e-dev >/dev/null; fi
	
		if dpkg --list | grep "libicu-dev" >/dev/null; then echo "'libicu-dev' already installed" >/dev/null;
		else apt-get install -y libicu-dev >/dev/null; fi
	
		if dpkg --list | grep "libsasl2-2" >/dev/null; then echo "'libsasl2-2' already installed" >/dev/null;
		else apt-get install -y libsasl2-2 >/dev/null; fi
		if dpkg --list | grep "libsasl2-dev" >/dev/null; then echo "'libsasl2-dev' already installed" >/dev/null;
		else apt-get install -y libsasl2-dev >/dev/null; fi
		if dpkg --list | grep "libsasl2-modules" >/dev/null; then echo "'libsasl2-modules' already installed" >/dev/null;
		else apt-get install -y libsasl2-modules >/dev/null; fi
	
		if dpkg --list | grep "libmcrypt4" >/dev/null; then echo "'libmcrypt4' already installed" >/dev/null;
		else apt-get install -y libmcrypt4 >/dev/null; fi
		if dpkg --list | grep "libmcrypt-dev" >/dev/null; then echo "'libmcrypt-dev' already installed" >/dev/null;
		else apt-get install -y libmcrypt-dev >/dev/null; fi
	
		if dpkg --list | grep "unixodbc-dev" >/dev/null; then echo "'unixodbc-dev' already installed" >/dev/null;
		else apt-get install -y unixodbc-dev >/dev/null; fi
	
		if dpkg --list | grep "libmysqlclient15-dev" >/dev/null; then echo "'libmysqlclient15-dev' already installed" >/dev/null;
		else apt-get install -y libmysqlclient15-dev >/dev/null; fi
	
		if dpkg --list | grep "libpq-dev" >/dev/null; then echo "'libpq-dev' already installed" >/dev/null;
		else apt-get install -y libpq-dev >/dev/null; fi
	
		if dpkg --list | grep "libpspell-dev" >/dev/null; then echo "'libpspell-dev' already installed" >/dev/null;
		else apt-get install -y libpspell-dev >/dev/null; fi
	
		if dpkg --list | grep "libsnmp-dev" >/dev/null; then echo "'libsnmp-dev' already installed" >/dev/null;
		else apt-get install -y libsnmp-dev >/dev/null; fi
	
		if dpkg --list | grep "libxslt1-dev" >/dev/null; then echo "'libxslt1-dev' already installed" >/dev/null;
		else apt-get install -y libxslt1-dev >/dev/null; fi
		if dpkg --list | grep "libxslt1.1" >/dev/null; then echo "'libxslt1.1' already installed" >/dev/null;
		else apt-get install -y libxslt1.1 >/dev/null ; fi

		if [ $INST_MEMCACHED == 1 ] 
			then
				if dpkg --list | grep "libevent" >/dev/null; then echo "'libevent' already installed" >/dev/null;
				else apt-get install -y libevent; fi
				if dpkg --list | grep "libevent-dev" >/dev/null; then echo "'libevent-dev' already installed" >/dev/null;
				else apt-get install -y libevent-dev; fi
				if dpkg --list | grep "memcached" >/dev/null; then echo "'memcached' already installed" >/dev/null;
				else apt-get install -y memcached; fi
				if dpkg --list | grep "libmemcached" >/dev/null; then echo "'libmemcached' already installed" >/dev/null;
				else apt-get install -y libmemcached; fi
				if dpkg --list | grep "libmemcached-devel" >/dev/null; then echo "'libmemcached-devel' already installed" >/dev/null;
				else apt-get install -y libmemcached-devel; fi
		fi
	fi
	...
	logMessage ">>> finish installing basic tools and/or libraries needed for build process...";
}
...
fetch_php () {
	...
	if [ -d "${INST_SRCURL}" ] && [ -f "${INST_SRCURL}/php5.spec.in" ]; then
		...
		if [ "$INST_VERBOSE" == "1" ]; then
			...
			if [ "$INST_KEEPARCHIVE" != "1" ]; then rm -rf ${INST_SRCURL}; logMessage ">>> remove source archive in '${INST_SRCURL}'..."; fi
			cd php-${INST_VERSION};
	    else
		    tar xzvf ${INST_SRCURL} ${INST_SRCPATH}/. >/dev/null;
			if [ "$INST_KEEPARCHIVE" != "1" ]; then rm -rf ${INST_SRCURL} >/dev/null; logMessage ">>> remove source archive in '${INST_SRCURL}'..."; fi
			cd php-${INST_VERSION};
		fi
		...
		if [ "$INST_VERBOSE" == "1" ]; then
			...
			if [ "$INST_KEEPARCHIVE" != "1" ]; then rm php-${INST_VERSION}.tar.gz; logMessage ">>> remove source archive in 'php-${INST_VERSION}.tar.gz'..."; fi
			cd php-${INST_VERSION};
	    else
		    wget ${INST_SRCURL} -O php-${INST_VERSION}.tar.gz >/dev/null;
		    tar xzvf php-${INST_VERSION}.tar.gz >/dev/null;
			if [ "$INST_KEEPARCHIVE" != "1" ]; then rm php-${INST_VERSION}.tar.gz >/dev/null; logMessage ">>> remove source archive in 'php-${INST_VERSION}.tar.gz'..."; fi
			cd php-${INST_VERSION};
		fi
	else
		if [ "$INST_VERBOSE" == "1" ]; then 
...
build_php () {
	...
	if [ "$INST_VERBOSE" == "1" ]; then 
		...
		## ./configure '--with-libdir=lib64' '--cache-file=../config.cache' --prefix=${INST_PATH} --with-config-file-path=${INST_INIPATH} --with-config-file-scan-dir=${INST_INIPATH}php.d '--disable-debug' '--with-pic' '--disable-rpath' '--with-bz2' '--with-curl' --with-freetype-dir=${INST_PATH} --with-png-dir=${INST_PATH} '--enable-gd-native-ttf' '--without-gdbm' '--with-gettext' '--with-gmp' '--with-iconv' --with-jpeg-dir=${INST_PATH} '-with-openssl' '--with-pspell' '--with-pcre-regex' '--with-zlib' '--enable-exif' '--enable-ftp' '--enable-sockets' '--enable-sysvsem' '--enable-sysvshm' '--enable-sysvmsg' '--enable-wddx' '--with-kerberos' '--with-unixODBC=/usr' '--enable-shmop' '--enable-calendar' '--without-sqlite3' --with-libxml-dir=${INST_PATH} '--enable-pcntl' '--with-imap' '--with-imap-ssl' '--enable-mbstring' '--enable-mbregex' '--with-gd' '--enable-bcmath' '--with-xmlrpc' '--with-ldap' '--with-ldap-sasl' '--with-mysql=/usr/' '--with-mysqli=/usr/bin/mysql_config' '--with-pdo-mysql=/usr/bin/mysql_config' '--with-pdo-pgsql' '--with-snmp' '--enable-soap' '--with-xsl' '--enable-xmlreader' '--enable-xmlwriter' '--enable-pdo' --with-pear=${INST_PATH}pear '--with-mcrypt' '--enable-intl' '--without-pdo-sqlite' '--enable-zip' '--enable-dba'
		...
		## ./configure '--with-libdir=/lib64' '--cache-file=../config.cache' --prefix=${INST_PATH} --with-config-file-path=${INST_INIPATH} --with-config-file-scan-dir=${INST_INIPATH}php.d ${INST_PHPCONFIGURE}  >/dev/null;
		## ./configure '--with-libdir=/lib64' '--cache-file=../config.cache' --prefix=${INST_PATH} --with-config-file-path=${INST_INIPATH} --with-config-file-scan-dir=${INST_INIPATH}php.d '--disable-debug' '--with-pic' '--disable-rpath' '--with-bz2' '--with-curl' --with-freetype-dir=${INST_PATH} --with-png-dir=${INST_PATH} '--enable-gd-native-ttf' '--without-gdbm' '--with-gettext' '--with-gmp' '--with-iconv' --with-jpeg-dir=${INST_PATH} '-with-openssl' '--with-pspell' '--with-pcre-regex' '--with-zlib' '--enable-exif' '--enable-ftp' '--enable-sockets' '--enable-sysvsem' '--enable-sysvshm' '--enable-sysvmsg' '--enable-wddx' '--with-kerberos' '--with-unixODBC=/usr' '--enable-shmop' '--enable-calendar' '--without-sqlite3' --with-libxml-dir=${INST_PATH} '--enable-pcntl' '--with-imap' '--with-imap-ssl' '--enable-mbstring' '--enable-mbregex' '--with-gd' '--enable-bcmath' '--with-xmlrpc' '--with-ldap' '--with-ldap-sasl' '--with-mysql' '--with-mysqli' '--with-pdo-mysql' '--with-pdo-pgsql' '--with-snmp' '--enable-soap' '--with-xsl' '--enable-xmlreader' '--enable-xmlwriter' '--enable-pdo' --with-pear=${INST_PATH}pear '--with-mcrypt' '--enable-intl' '--without-pdo-sqlite' '--enable-zip' '--enable-dba' '--enable-memcached'  >/dev/null
		## ./configure '--with-libdir=lib64' '--cache-file=../config.cache' --prefix=${INST_PATH} --with-config-file-path=${INST_INIPATH} --with-config-file-scan-dir=${INST_INIPATH}php.d '--disable-debug' '--with-pic' '--disable-rpath' '--with-bz2' '--with-curl' --with-freetype-dir=${INST_PATH} --with-png-dir=${INST_PATH} '--enable-gd-native-ttf' '--without-gdbm' '--with-gettext' '--with-gmp' '--with-iconv' --with-jpeg-dir=${INST_PATH} '-with-openssl' '--with-pspell' '--with-pcre-regex' '--with-zlib' '--enable-exif' '--enable-ftp' '--enable-sockets' '--enable-sysvsem' '--enable-sysvshm' '--enable-sysvmsg' '--enable-wddx' '--with-kerberos' '--with-unixODBC=/usr' '--enable-shmop' '--enable-calendar' '--without-sqlite3' --with-libxml-dir=${INST_PATH} '--enable-pcntl' '--with-imap' '--with-imap-ssl' '--enable-mbstring' '--enable-mbregex' '--with-gd' '--enable-bcmath' '--with-xmlrpc' '--with-ldap' '--with-ldap-sasl' '--with-mysql=/usr/' '--with-mysqli=/usr/bin/mysql_config' '--with-pdo-mysql=/usr/bin/mysql_config' '--with-pdo-pgsql' '--with-snmp' '--enable-soap' '--with-xsl' '--enable-xmlreader' '--enable-xmlwriter' '--enable-pdo' --with-pear=${INST_PATH}pear '--with-mcrypt' '--enable-intl' '--without-pdo-sqlite' '--enable-zip' '--enable-dba'  >/dev/null
	...
	if [ "$INST_VERBOSE" == "1" ]; then echo ">>> ini file created in '${INST_INIPATH}php.ini'..."; fi
	logMessage ">>> ini file created in '${INST_INIPATH}php.ini'..."
	
	
	if [ "$INST_VERBOSE" == "1" ]; then
		echo ">>> remove source files in '${INST_SRCPATH}'...";
		if [ "$INST_KEEPFILES" == "0" ]; then rm -rf ${INST_SRCPATH}; logMessage ">>> remove source files in '${INST_SRCPATH}'..."; fi
	else
		if [ "$INST_KEEPFILES" == "0" ]; then rm -rf ${INST_SRCPATH} >/dev/null; logMessage ">>> remove source files in '${INST_SRCPATH}'..."; fi
	fi
	...
	if [ "$INST_VERBOSE" == "1" ]; then
		...
		if [ -d "${INST_SUHOSINURL}" ] && [ -f "${INST_SUHOSINURL}/suhosin.c" ]; then
			...
			if [ "$INST_VERBOSE" == "1" ]; then
				...
				if [ "$INST_KEEPARCHIVE" != "1" ]; then rm -rf ${INST_SUHOSINURL}; logMessage ">>> remove source archive in '${INST_SUHOSINURL}'..."; fi
				cd suhosin-${INST_SUHOSINVERSION};
		    else
			    ## tar xzvf ${INST_SUHOSINURL} ${INST_SUHOSINPATH}/. >/dev/null;
				if [ "$INST_KEEPARCHIVE" != "1" ]; then rm -rf ${INST_SUHOSINURL} >/dev/null; logMessage ">>> remove source archive in '${INST_SUHOSINURL}'..."; fi
				cd suhosin-${INST_SUHOSINVERSION};
			fi
			...
			if [ "$INST_VERBOSE" == "1" ]; then
				...
				if [ "$INST_KEEPARCHIVE" != "1" ]; then rm suhosin-${INST_SUHOSINVERSION}.tar.gz; logMessage ">>> remove source archive in 'php-${INST_VERSION}.tar.gz'..."; fi
				cd suhosin-${INST_SUHOSINVERSION};
		    else
				...
				if [ "$INST_KEEPARCHIVE" != "1" ]; then rm suhosin-${INST_SUHOSINVERSION}.tar.gz >/dev/null; logMessage ">>> remove source archive in 'php-${INST_VERSION}.tar.gz'..."; fi
				cd suhosin-${INST_SUHOSINVERSION};
			fi
		else
			if [ "$INST_VERBOSE" == "1" ]; then 
	...
	if [ "$INST_VERBOSE" == "1" ]; then
		...
		if [ -d "${INST_XDEBUGURL}" ] && [ -f "${INST_XDEBUGURL}/xdebug.c" ]; then
			...
			if [ "$INST_VERBOSE" == "1" ]; then
				...
				if [ "$INST_KEEPARCHIVE" != "1" ]; then rm -rf ${INST_XDEBUGURL}; logMessage ">>> remove source archive in '${INST_XDEBUGURL}'..."; fi
				cd xdebug-${INST_XDEBUGVERSION};
		    else
			    ## tar xzvf ${INST_XDEBUGURL} ${INST_XDEBUGPATH}/. >/dev/null;
				if [ "$INST_KEEPARCHIVE" != "1" ]; then rm -rf ${INST_XDEBUGURL} >/dev/null; logMessage ">>> remove source archive in '${INST_XDEBUGURL}'..."; fi
				cd xdebug-${INST_XDEBUGVERSION};
			fi
			...
			if [ "$INST_VERBOSE" == "1" ]; then
				...
				if [ "$INST_KEEPARCHIVE" != "1" ]; then rm xdebug-${INST_XDEBUGVERSION}.tar.gz; logMessage ">>> remove source archive in 'xdebug-${INST_XDEBUGVERSION}.tar.gz'..."; fi
				cd xdebug-${INST_XDEBUGVERSION};
		    else
				...
				if [ "$INST_KEEPARCHIVE" != "1" ]; then rm xdebug-${INST_XDEBUGVERSION}.tar.gz >/dev/null; logMessage ">>> remove source archive in 'xdebug-${INST_XDEBUGVERSION}.tar.gz'..."; fi
				cd xdebug-${INST_XDEBUGVERSION};
			fi
		else
			if [ "$INST_VERBOSE" == "1" ]; then 
	...
	if [ "$INST_VERBOSE" == "1" ]; then
		...
		if [ -d "${INST_MEMCACHEDURL}" ] && [ -f "${INST_MEMCACHEDURL}/memcached.c" ]; then
			...
			if [ "$INST_VERBOSE" == "1" ]; then
				...
				if [ "$INST_KEEPARCHIVE" != "1" ]; then rm -rf ${INST_MEMCACHEDURL}; logMessage ">>> remove source archive in '${INST_MEMCACHEDURL}'..."; fi
				...
				if [ "$INST_KEEPARCHIVE" != "1" ]; then rm -rf ${INST_MEMCACHEDURL} >/dev/null; logMessage ">>> remove source archive in '${INST_MEMCACHEDURL}'..."; fi
				chown -R root:root memcached-${INST_MEMCACHEDVERSION};
				cd memcached-${INST_MEMCACHEDVERSION};
			fi
			...
			if [ "$INST_VERBOSE" == "1" ]; then
				...
				if [ "$INST_KEEPARCHIVE" != "1" ]; then rm memcached-${INST_MEMCACHEDVERSION}.tar.gz; logMessage ">>> remove source archive in 'memcached-${INST_MEMCACHEDVERSION}.tar.gz'..."; fi
				chown -R root:root memcached-${INST_MEMCACHEDVERSION};
				cd memcached-${INST_MEMCACHEDVERSION};
		    else
				...
				if [ "$INST_KEEPARCHIVE" != "1" ]; then rm memcached-${INST_MEMCACHEDVERSION}.tar.gz >/dev/null; logMessage ">>> remove source archive in 'memcached-${INST_MEMCACHEDVERSION}.tar.gz'..."; fi
				chown -R root:root memcached-${INST_MEMCACHEDVERSION};
				cd memcached-${INST_MEMCACHEDVERSION};
			fi
		else
